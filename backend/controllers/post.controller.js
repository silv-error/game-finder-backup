...
export const getAllPosts = async (req, res) => {
  try {
    const excludePost = await Post.find().sort({ createdAt: -1 }).limit(3);
...
export const getLatestPost = async (req, res) => {
  try {
    const post = await Post.find({}).sort({ createdAt: -1 }).populate({ path: "user", select: "-password" }).limit(3);
...
export const deletePost = async (req, res) => {
  try {
    const { id } = req.params;
    const post = await Post.findByIdAndDelete(id, { user: req.user._id });
    if (!post) return res.status(400).json({ error: "Failed to delete post" });
