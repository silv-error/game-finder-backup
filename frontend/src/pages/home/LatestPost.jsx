    ...
    <>
      ...
      <div className="flex justify-between md:mt-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 mt-2 md:gap-4 md:mt-2">
          ...
          {!myPost && (
            <Link to={`/chat/${post.user?._id}`} onClick={() => setSelectedConversation(post.user)}>
              <button
                onClick={() => setSelectedConversation(post.user)}
                className="btn btn-xs sm:btn-sm md:btn-md btn-primary text-slate-100 rounded-md"
              >
                Connect
              </button>
            </Link>
      ...
      <dialog id={post._id} className="modal">
        <div className="modal-box">
          ...
          <h3 className="font-bold text-lg">Invite Details</h3>
          ...
          <div className="grid grid-cols-1">
            ...
            {authUser._id === post.user._id && (
              <button
                ...
                className="btn btn-md hover:text-red-500 w-full mt-4"
              >
                {postId ? <LoadingState size={"sm"} /> : <Trash size={20} />}
              </button>
            )}
          </div>
        </div>
